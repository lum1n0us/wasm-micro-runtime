...
def _known_suites(unit_dir: str):
    """Suite directories under tests/unit (both the parent suites and, when
    FULL_TEST is on, every suite nested under llm-enhanced-test).
    ...
    The submodule groups some suites in subdirectories (e.g. the POSIX suite
    is split into llm-enhanced-test/posix/platform and
    llm-enhanced-test/posix/libc-wasi), so the llm suites are discovered
    recursively.

    Returns (parent_names, llm_rel_names).  llm_rel_names holds suite paths
    relative to tests/unit/llm-enhanced-test ('wasm-vm', 'posix/platform',
    ...)."""
    ...
    parent = _cmake_suites(unit_dir)
    llm_root = os.path.join(unit_dir, "llm-enhanced-test")
    llm = set()
    if os.path.isdir(llm_root):
        for root, dirs, files in os.walk(llm_root):
            dirs[:] = [d for d in dirs if not d.startswith(".")]
            if root == llm_root or "CMakeLists.txt" not in files:
                continue
            rel = os.path.relpath(root, llm_root).replace(os.sep, "/")
            llm.add(rel)
    return parent, llm
...
def classify_targets(units: List[CompileUnit], unit_dir: str) -> Dict[str, List[CompileUnit]]:
    ...
    nested "llm-enhanced-test/<suite>" (possibly deeper, e.g.
    "llm-enhanced-test/posix/platform") for the llm-enhanced-test submodule
    ...
    def llm_suite(path_parts: List[str]) -> str:
        """Longest known llm suite path matching the leading path parts."""
        for j in range(len(path_parts), 0, -1):
            cand = "/".join(path_parts[:j])
            if cand in llm_names:
                return cand
        return ""
    ...
    for u in units:
        ...
        if key is None:
            ...
            # <build>/llm-enhanced-test/<suite>[/<subsuite>]/... for the
            # submodule ones).  Pick the *deepest* suite-looking segment so an
            # llm suite that shares a name with a parent suite is not
            # confused with it.
            segs = u.directory.split(os.sep)
            best, best_idx = None, -1
            for i, part in enumerate(segs):
                if part == "llm-enhanced-test":
                    suite = llm_suite(segs[i + 1:])
                    if suite:
                        end_idx = i + len(suite.split("/"))
                        if end_idx > best_idx:
                            best, best_idx = llm_key(suite), end_idx
...
if __name__ == "__main__":
    import sys
    from coverage_features import expand_features
