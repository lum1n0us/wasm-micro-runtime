...
{
struct ili9340_data *data = &ili9340_data1; //(struct ili9340_data *)dev->driver_data;
...
{
struct ili9340_data *data = (struct ili9340_data *) &ili9340_data1; //(struct ili9340_data *)dev->driver_data;
...
//		for(i=0;i<tx_len;i++){			printf("%x ",buf1[i]);		}
    //spi_write(data->spi_dev, &data->spi_config, &tx_bufs);
    spi_transceive(data->spi_dev, &data->spi_config, &tx_bufs, NULL);
}
//	printf("\n");
