...
# Pre-requisites
sudo apt -q update
sudo apt install -y -q build-essential cmake g++-multilib libgcc-12-dev lib32gcc-12-dev ccache ninja-build
...
iwasm_options_list=(
    ...
    "-DWAMR_BUILD_JIT=1"
    # +fast jit
    "-DWAMR_BUILD_FAST_JIT=1 -DWAMR_BUILD_FAST_JIT_DUMP=1"
    #
    #
    "-DWAMR_BUILD_TARGET=X86_32"
    #
    # libraries
    "-DWAMR_BUILD_LIBC_BUILTIN=0 -DWAMR_BUILD_LIBC_UVWASI=1"
    ...
    #
    # Wasm specs
    "-DWAMR_BUILD_GC=1 -DWAMR_BUILD_EXCE_HANDLING=1"
    ...
    # - Debug
    "-DWAMR_BUILD_DEBUG_INTERP=1 -DWAMR_BUILD_DEBUG_AOT=1"
    # - name section
    "-DWAMR_BUILD_CUSTOM_NAME_SECTION=1  -DWAMR_BUILD_LOAD_CUSTOM_SECTION=1 -DWAMR_BUILD_DUMP_CALL_STACK=1"
    # - profiling
    "-DWAMR_BUILD_MEMORY_PROFILING=1 -DWAMR_BUILD_PERF_PROFILING=1"
    #
    ...
    "-DWAMR_DISABLE_WAKEUP_BLOCKING_OP=1"
    "-DWAMR_BUILD_MODULE_INST_CONTEXT=0"
    "-DWAMR_BUILD_STATIC_PGO=1"
    "-DWAMR_CONFIGURABLE_BOUNDS_CHECKS=1"
    "-DWAMR_BUILD_LINUX_PERF=1"
    "-DWAMR_BUILD_DYNAMIC_AOT_DEBUG=1"
)
