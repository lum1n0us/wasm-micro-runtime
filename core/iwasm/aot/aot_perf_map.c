                    ...
                    uint32 error_buf_size)
{
    ...
    (void)snprintf(perf_map_path, sizeof(perf_map_path) - 1, "/tmp/perf-%d.map", pid);
    ...
    for (i = 0; i < module->func_count; i++) {
        ...
        if (strlen(module_name) > 0) {
            (void)snprintf(perf_map_info, 128, "%" PRIxPTR "  %x  [%s]#aot_func#%u\n",
                     (uintptr_t)sorted_func_ptrs[i].ptr,
                     get_func_size(module, sorted_func_ptrs, i), module_name,
                     sorted_func_ptrs[i].idx);
        }
        else {
            (void)snprintf(perf_map_info, 128, "%" PRIxPTR "  %x  aot_func#%u\n",
                     (uintptr_t)sorted_func_ptrs[i].ptr,
