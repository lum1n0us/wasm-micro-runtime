...
void bh_feed_transaction(void * transaction, void * response, uint32_t len,
        uint8_t format)
{
    ...
    if (node->sync_type == 1) {
        ...
        if (len) {
            ...
            if (node->sync_data.response == NULL) {
                ...
                bh_memcpy_s(node->sync_data.response, len, response, len);
            }
        }
        ...
        if (cb == NULL) {
            PRINTX("trans: malloc fail\n");
            free(node->cb_data.context_data);
            free(node);
            return;
        }
        ...
        if (len) {
            ...
            bh_memcpy_s(cb->data, len, response, len);
        } else {
...
/*bh_async_callback*/void* cb, void* ctx_data, uint32_t timeout,
        void * worker_thread)
{
...
static uint32_t bh_remove_expired_trans(sync_ctx_t* sync_ctx,
        sync_node_t ** expired_list)
{
    bh_bsp_lock(sync_ctx->ctx_lock);
    uint32_t elpased_ms = bh_get_elpased_ms((uint32_t*) &sync_ctx->last_check);
